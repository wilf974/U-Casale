...
export default function EditGitePage() {
  const router = useRouter()
  const params = useParams()
  const giteId = params.id as string
  ...
  const fetchGite = async () => {
    try {
      const response = await fetch(`/api/admin/gites/${giteId}`)
      const data = await response.json()

      if (response.ok && data.gite) {
        const gite = data.gite
        setFormData({
          name: gite.name || "",
          slug: gite.slug || "",
          description: gite.description || "",
          shortDescription: gite.shortDescription || "",
          maxGuests: String(gite.maxGuests || 4),
          bedrooms: String(gite.bedrooms || 2),
          beds: String(gite.beds || 2),
          bathrooms: String(gite.bathrooms || 1),
          pricePerNight: String(gite.pricePerNight || ""),
          cleaningFee: String(gite.cleaningFee || 0),
          minimumStay: String(gite.minimumStay || 1),
          address: gite.address || "",
          city: gite.city || "",
          postalCode: gite.postalCode || "",
          featuredImage: gite.featuredImage || "",
          virtualTourUrl: gite.virtualTourUrl || "",
          equipment: gite.equipment || "",
          available: gite.available !== undefined ? gite.available : true,
          rules: gite.rules || "",
          checkInTime: gite.checkInTime || "15:00",
          checkOutTime: gite.checkOutTime || "11:00",
          metaTitle: gite.metaTitle || "",
          metaDescription: gite.metaDescription || "",
          displayOrder: String(gite.displayOrder || 0),
          featured: gite.featured || false,
        })
        setImages(gite.images || [])
        setAmenities(gite.amenities || {})
      }
    } catch (error) {
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h1 className="text-3xl font-bold text-stone-900">Modifier le gîte</h1>
        </div>
        <button
